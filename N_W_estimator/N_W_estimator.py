...
class KernelRegression(BaseEstimator, RegressorMixin):
    ...
    def predict(self, x_test, y_test):
        ...
        acc          = torch.sum(y_class == p_class).item() / len(y_class)
        # print(eval_metrics)
    ...
    def forward(self, x_test, y_test):
        # block = self.block
        output, acc = self.predict(x_test, y_test)
        return output, acc, self.M
...
if __name__ == "__main__":
    ...
    ### load datasets that are preprocessed with ZCA and Aug
    # x_train, y_train = cifar.load_train_dataset()
    # x_test, y_test   = cifar.load_test_dataset()
    ...
    x_train_part1   = x_train[0:50000]
    x_train_part2   = x_train[0:50000]
    x_train_part3   = x_train[20000:30000]
    x_train_part4   = x_train[30000:40000]
    x_train_part5   = x_train[40000:50000]
    
    y_train_part1   = y_train[0:50000]
    y_train_part2   = y_train[0:50000]
    ...
    best_acc = 0.0
    best_gamma = 1
    GAMMA_LIST = [0.1 * (2.0 ** i)for i in range(-20, 20)]
    for gamma_layer1 in GAMMA_LIST:
        print('gamma1 ', gamma_layer1)
        kr              = KernelRegression(kernel="rbf", gamma=gamma_layer1)
        y_kr, acc, M    = kr.fit(x_train_part1, y_train_part1).forward(x_test, y_test) # 1 layer
        # y_class         =  torch.argmax(y_test, dim=-1)
    ...
    # #### dtd
    # n_class = 10
    # (x_train, y_train), (x_test, y_test) = dtd.load_10classes()
    # kr = KernelRegression(kernel="rbf", gamma=0.3)
