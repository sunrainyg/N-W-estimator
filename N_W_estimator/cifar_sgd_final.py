...
class udfNetWork:
    def __init__(self, batch_size, data_x, lr = 0.0001):

        self.W = torch.eye(3072)

        data_x = torch.reshape(data_x, (1, 3072))
        self.t = data_x.repeat(batch_size, 1)
    ...
    def update(self, x, y):
        ...
        grad_c.clamp_(min=0, max=1)
        self.c = self.c - self.learning_rate * grad_c

        xt = x - self.t

        grad_t = 4 * (self.c @ delta.T @ g_out @ xt @ self.W.T @ self.W).sum(0)
        grad_t.clamp_(min=0, max=1)
        self.t = self.t - self.learning_rate * grad_t

        grad_m = -4 * (self.W @ (xt.T @ (self.c @ delta.T @ g_out @ xt))).sum(0)
        grad_m.clamp_(min=0, max=1)
        self.M = self.W - self.learning_rate * grad_m


        #print(grad_c, grad_t, grad_m)
...
def train():
    ...
    data_x, data_y = zip(*trainset)
    model = udfNetWork(batch_size, data_x[0])
