...
if args.net=='res18':
    ...
    # ViT for cifar10
    print("-------we are using vit for cifar10--------")
    net = ViT(
    image_size = size,
    patch_size = args.patch,
    num_classes = 10,
...
def plot_M_img_2(M):
        ...
        pcm = axarr[0].imshow(torch.mean(torch.diag(M.cpu()).unsqueeze(0).expand(3, -1, -1),axis=0).detach().numpy(),cmap='cividis')
...
def plot_M_img(M):
    ...
    M = M[448:512,:].cpu().detach().numpy()
    plt.figure(figsize=(10, 6))
    plt.imshow(M, cmap='viridis', aspect='auto')
    plt.colorbar(label='Value')
    plt.title('Heatmap of a 512x64 Vector')
    plt.xlabel('Dimension')
    plt.ylabel('Index')
    plt.savefig("M_vis_222.jpg")
    plt.close()
