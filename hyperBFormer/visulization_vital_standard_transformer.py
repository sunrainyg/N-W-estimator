...
class RBFNetwork(nn.Module):
    ...
    def forward(self, x): 
        # # Compute the squared norms for x and centres
        # x_norm = (x ** 2).sum(dim=-1, keepdim=True)
        # centre_norm = (self.centres ** 2).sum(dim=-1)

        # # Compute the product of x and centres
        # prod = torch.matmul(x, self.centres.T)

        # # Using the formula above
        # distances_sq = x_norm + centre_norm - 2 * prod
        # distances = distances_sq.sqrt()
        
        ## 马氏距离版本
        # 1. Compute q^T M q
        q_M = q @ self.M_RBF
        qMq = (q * q_M).sum(dim=-1, keepdim=True)
        # 2. Compute k^T M k
        k_M = k @ self.M_RBF
        kMk = (k * k_M).sum(dim=-1, keepdim=True).transpose(-2, -1)
        # 3. Compute -2 q^T M k
        qMk = q_M @ k.transpose(-2, -1)
        negative_2qMk = -2 * qMk
        # Combine all components to get Mahalanobis distance
        distances = qMq + kMk + negative_2qMk

        # RBF activation
        activations = torch.exp(-distances ** 2 / (2 * self.sigma ** 2))
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=GaussianActivation, drop=0.):
        super().__init__()
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        ## remove mlp, but replace with attn. layers
        x, _, _ = self.attn(self.norm2(x))
...
if __name__ == "__main__":
    ...
    if args.vis:
        ...
        ma              = visualize_predict(model, img, img_size, patch_size, device)
