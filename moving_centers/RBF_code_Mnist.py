...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2048, shuffle=False)
...
class RBFnet(nn.Module):
    def __init__(self, clusters):
        ...
        self.M    = nn.Parameter( torch.eye(784,dtype=torch.float64, requires_grad=True))
...
def train(model, train_loader, optimizer, criterion):
    ...
    return train_acc, train_loss, M_matrix
...
for epoch in range(num_epochs):
    train_acc, train_loss, M_matrix = train(model, train_loader, optimizer, criterion)
