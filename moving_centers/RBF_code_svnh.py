...
# Load svhn dataset
train_dataset = torchvision.datasets.SVHN(root='./data', download=True, transform=transform)
test_dataset = torchvision.datasets.SVHN(root='./data', download=True, transform=transform)

train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=512, shuffle=False)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=512, shuffle=False)

train_data = datasets.SVHN(root='./data', transform=transform)
data = torch.tensor(train_data.data)
X_train = data.reshape(-1, 3072).double().to(device) / 255.0
...
class RBFnet(nn.Module):
    ...
    def forward(self, x):
        ...
        madistances     = madistances.clamp(min=0).sqrt_()
        #######################################

        # Calculate the Gaussian activations
        res                 = torch.exp((-0.5) * (madistances**2) / self.sigs**2)
        # Set any NaN values to 0 (in case self.sigs is zero)
        nan_mask            = torch.isnan(res)
        # pdb.set_trace()
        res                 = torch.where(nan_mask, torch.tensor(0.0, dtype=res.dtype).to('cuda'), res)
...
def test(model, test_loader, optimizer, criterion):
    ...
    with torch.no_grad():
        for i, (images_t, labels_t) in enumerate(test_loader):
            ...
            # Forward pass
            outputs_t, M_matrix, centers = model(images_t)
...
def plot_M_img(M, train_dataset, i):
    ...
    pcm = axarr[0].imshow(torch.mean(torch.diag(M.cpu()).reshape(3,32,32).unsqueeze(0).expand(3, -1, -1),axis=0).detach().numpy(),cmap='cividis')
    axarr[0].set_title("M matrix diagonal")
    f.colorbar(mappable=pcm, ax=axarr[0], shrink=0.8,location="left")
    axarr[1].imshow(torch.moveaxis(train_dataset[5][0],0,2))
...
plt.title('t-SNE')
plt.savefig('cluster_centers_visualization.png')