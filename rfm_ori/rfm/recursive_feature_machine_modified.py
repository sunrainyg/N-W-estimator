...
from .kernels import EuclideanDistancesM, LaplacianM
...
class RecursiveFeatureMachine(torch.nn.Module):
    ...
    def __init__(self, device=torch.device('cpu'), mem_gb=32, diag=False, centering=False, reg=1e-3):
        super().__init__()
        self.M = torch.nn.Parameter(torch.eye(3072, 3072, dtype=torch.float32), requires_grad=True)
        self.centers = torch.nn.Parameter(torch.ones(50000, 3072, dtype=torch.float32), requires_grad=True)
        self.weights = torch.nn.Parameter(0.001 * torch.ones(50000, 10, dtype=torch.float32), requires_grad=True)
        self.model = None
...
class LaplaceRFM(RecursiveFeatureMachine):
    ...
    def __init__(self, bandwidth=1., **kwargs):
        ...
        self.laplacian_M = LaplacianM(self.bandwidth)
        self.kernel = lambda x, z: self.laplacian_M(x, z, self.M) # must take 3 arguments (x, z, M)
...
if __name__ == "__main__":
    ...
    def fit(self, train_loader, test_loader,
        ...
        for i in range(iters):
            ...
            optimizer.zero_grad()
            pred = self.predict(self.centers, self.weight)
            criterion = MSEloss()
